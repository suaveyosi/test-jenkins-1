...
pipeline {
 ...
 stages {
        stage('Checkout-git'){
               steps{
		git poll: true, url: 'https://github.com/suaveyosi/test-jenkins-1.git'
               }
        }
...
#            		bash -c "source ${WORKSPACE}/entorno_virtual/bin/activate && ${WORKSPACE}/entorno_virtual/bin/python ${WORKSPACE}/entorno_virtual/bin/pip install -r requirements.txt"
    ...
    stage('PushDockerImage') {
            steps {
            	sh '''
            		docker tag apptest:latest suaveyosi/apptest:latest
					docker push suaveyosi/apptest:latest
